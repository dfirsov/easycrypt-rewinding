...
proof. have dw : weight (drestrict X M) = (mu X M). smt.
rewrite - dw.
apply dscalar_ll.
qed.
...
proof.  move => mpos he.
rewrite /hasE.
  have ->: (fun (x : 'a) => g x * mass (inv (mu X M) \cdot drestrict X M) x)
      =  (fun (x : 'a) => g x * inv (mu X M) * mass (drestrict X M) x).
  apply fun_ext.
  move => x.
  rewrite massE.
  rewrite dscalar1E. split. smt.  move => p.
  have ->: mu X M = weight (drestrict X M). timeout 20. smt.
  auto. smt.
  have : hasE (drestrict X M) (fun x => inv (mu X M) * g x).
  apply hasEZ. smt.
rewrite /hasE.
simplify. auto.
qed.
...
have co : mu X (predC M) = 1%r - mu X M. smt.
rewrite co.
...
have dfin : is_finite (support (inv (mu X M) \cdot drestrict X M)).
   ...
   have Mx : M x. smt.
   have xX : x \in X. smt.
   have xXM : x \in (drestrict X M). smt.
   apply supp_dscalar. smt.
...
have  xiX : forall x, x \in (inv (mu X (predC M)) \cdot drestrict X (predC M))
   ...
   smt. move => eq. smt.
   smt. smt.
   apply has_E.        smt. smt. progress. rewrite ec_edot. apply Jensen_inf4_gen_1. smt.
have  xiX : forall x, x \in (inv (mu X (predC M)) \cdot drestrict X (predC M))
   => x \in X.
   move => x pr.
   have : x \in (inv (mu X (predC M)) \cdot drestrict X (predC M)) <=> x \in drestrict X (predC M).
...
apply (ler_trans (E X (f \o g) -  mu X (predC M) * c + mu X (predC M) * d)).
   ...
   rewrite ec_edot.
   apply (Jensen_inf4_gen_2 (inv (mu X (predC M)) \cdot drestrict X (predC M)) (f \o g) ).  apply is_loss. smt.
have  xiX : forall x, x \in (inv (mu X (predC M)) \cdot drestrict X (predC M))
   => x \in X.
   move => x pr.
   have : x \in (inv (mu X (predC M)) \cdot drestrict X (predC M)) <=> x \in drestrict X (predC M).
